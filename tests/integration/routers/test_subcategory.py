...
class TestSubcategory(AssertMixin): 
    @pytest.mark.parametrize("case", testing_cases.TEST_SUBCATEGORY_ROUTER_GET)
    ...
    @pytest.mark.parametrize("case", testing_cases.TEST_SUBCATEGORY_ROUTER_CREATE)
    ...
    @pytest.mark.parametrize("case", testing_cases.TEST_SUBCATEGORY_ROUTER_UPDATE)
    async def test_update(self, case: RequestTestCase, async_client: AsyncClient) -> None: 
        response = await async_client.request(case.method, case.url, json=case.data, headers=case.headers)
        self.assertEqual(response.status_code, case.expected_status)
        self.assertEqual(prepare_payload(response), case.expected_data)

    @pytest.mark.parametrize("case", testing_cases.TEST_SUBCATEGORY_ROUTER_DELETE)
